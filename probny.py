...
def command_map():
    ...
    #
    # def show_sensors_data(selected_point):
    # # usunięcie zawartości listboxa:
        # listbox.delete(0, END)
        #
    ...
    def on_point_click(event):
        # pobranie nazwy klikniętego punktu
        selected_point = canvas.gettags(tk.CURRENT)[0]
        # usunięcie mapy
        canvas.delete("all")
        # utworzenie i wypełnienie listboxa
        listbox = Listbox(root)
        listbox.insert(tk.END, selected_point)
    ...
    for id, latitude, longitude in stations:
        ...
        canvas.after(1000, canvas.tag_bind, id, "<Button-1>", on_point_click)
